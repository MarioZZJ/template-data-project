...
本页保留旧版模板的固定历史，不是当前 Multica 执行规范。下文的手工看板、README 执行顺序、仓库内数据路径和目录初始化要求只描述当时快照；新项目遵循当前 [执行入口](../../AGENTS.md) 和 `INITIALIZE_PROJECT.md`。
...
| 历史示例做法 | 当前模板做法 |
|---|---|
| README 手工维护完整命令与依赖 | 科学依赖写研究计划，执行依赖在 Multica，实际命令进入运行记录 |
| 手工看板同步状态 | Multica 维护状态，DASHBOARD 按平台快照生成 |
| `data/raw/` 等仓库内忽略目录 | 外部 `ResearchAssets/<仓库名>/inputs/` 与各次运行 `data/` |
| 从工作树直接逐项运行 | `run` 固定实际代码和稳定环境，系统托管后自动交接 |
| 生成图表直接放正式目录 | 候选图表先留在运行目录，选择后 `export` 并维护来源索引 |
| 多份目录 README 填写项目配置 | 项目事实集中到简介、计划、偏好与数据来源 |
| 手工结束任务和启动下一项 | 完整交付经检查后自动完成子任务，阶段由用户验收 |

原快照、比较链接和具体研究结论保持不变，不为了匹配新流程改写历史。参考其科学问题、诊断和论文证据关系时，仍须重新判断是否适用于当前研究。
