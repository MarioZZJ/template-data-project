...
服务已登记并核验固定客户端时，`context` 输出还保留 `client_entrypoint` 的 `path`、`sha256`、`usage` 三个字段，方便从旧检出找到当前入口；不转发其他部署字段。这里仅展示入口信息，不执行它、不改本机映射，也不把服务直调成功当作客户端命令已经通过实测。CLI 回退仍按原路径提供平台事实。
